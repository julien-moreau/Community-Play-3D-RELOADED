...
AspectWidgetList::AspectWidgetList ()
{
}
...
void  AspectWidgetList::make (opak::GraphicContext & context)
{
   ...
   if (update_flag)
   {
      ...
      _item_text_arr [Selection_OFF] = aspect_widget.get_color_strong (.25f);
      _item_text_arr [Selection_ON] = aspect_widget.get_color_strong (1.f);
   }
}
...
void  AspectWidgetList::do_init (opak::GraphicContext & context)
{
   load_base_elements ();
...
void  AspectWidgetList::load_base_elements ()
{
   if (_be._item_back.size () == 0)
   {
      ...
      ok_flag = il.load (_be._item_back, "ohm.widget.list.item.back.png");
      assert (ok_flag);
   }
}
...
AspectWidgetList::BaseElements   AspectWidgetList::_be;
