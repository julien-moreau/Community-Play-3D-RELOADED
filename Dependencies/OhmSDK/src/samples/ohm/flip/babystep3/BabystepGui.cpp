...
void  BabystepGui::document_changed (ModelRoot & root)
{
   ...
   if (root.was_inited ())
   {
      bnd_ptr = new BundleRoot (root, _window.use_view ());

      _window.use_view ().embed (*bnd_ptr, opa::BundleView::Slot_WORKSPACE);
      
      // attach 'bnd_ptr' to 'root'
      
      root.reset_data (bnd_ptr);
   }
   else
   {
      bnd_ptr = root.get_data <BundleRoot> ();
   }
