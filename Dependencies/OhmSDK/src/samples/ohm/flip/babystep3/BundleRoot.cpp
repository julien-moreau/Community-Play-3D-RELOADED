...
,  _parent (parent)
{
   ...
   _prim_aptr->_point = opa::Point (
      _root.get_point_x (), _root.get_point_y ()
   );
}
...
void  BundleRoot::update ()
{
   ...
   if (_root.point_changed ())
   {
      _prim_aptr->_point
         .stop ()
         .push (
            opa::Point (_root.get_point_x (), _root.get_point_y ()),
            1.0, opak::ease_in_ease_out
         );
   }
}
...
Name : move
Description :
   Called from FrameRoor, this indicates that a user input occured, and that
