...
#include "Stdafx.h"
#include "CMeshSceneNodeInstances.h"
...
//! constructor
CMeshSceneNodeInstances::CMeshSceneNodeInstances(IMesh* mesh, ISceneNode* parent, ISceneManager* mgr, s32 id,
...
{
	#ifdef _DEBUG
	setDebugName("CMeshSceneNodeInstances");
	#endif
...
//! destructor
CMeshSceneNodeInstances::~CMeshSceneNodeInstances() {
	if (Mesh)
		Mesh->drop();
}
...
//! frame
void CMeshSceneNodeInstances::OnRegisterSceneNode() {
	if (IsVisible) {
...
//! renders the node.
void CMeshSceneNodeInstances::render() {
	video::IVideoDriver* driver = SceneManager->getVideoDriver();
...
bool CMeshSceneNodeInstances::removeChild(ISceneNode* child) {
	return ISceneNode::removeChild(child);
}


//! returns the axis aligned bounding box of this node
const core::aabbox3d<f32>& CMeshSceneNodeInstances::getBoundingBox() const {
...
video::SMaterial& CMeshSceneNodeInstances::getMaterial(u32 i) {
...
u32 CMeshSceneNodeInstances::getMaterialCount() const {
...
//! Sets a new mesh
void CMeshSceneNodeInstances::setMesh(IMesh* mesh) {
	if (mesh) {
		mesh->grab();
		if (Mesh)
			Mesh->drop();
...
IShadowVolumeSceneNode* CMeshSceneNodeInstances::addShadowVolumeSceneNode(
		const IMesh* shadowMesh, s32 id, bool zfailmethod, f32 infinity)
{
	// CMeshSceneNodeInstances is a CP3DR insternal feature.
	// Since CP3DR has its own shadows system, we return a null pointer.
	return 0;
}


void CMeshSceneNodeInstances::copyMaterials() {
	Materials.clear();
...
void CMeshSceneNodeInstances::serializeAttributes(io::IAttributes* out, io::SAttributeReadWriteOptions* options) const {
...
void CMeshSceneNodeInstances::deserializeAttributes(io::IAttributes* in, io::SAttributeReadWriteOptions* options) {
...
void CMeshSceneNodeInstances::setReadOnlyMaterials(bool readonly) {
	ReadOnlyMaterials = readonly;
}


//! Returns if the scene node should not copy the materials of the mesh but use them in a read only style
bool CMeshSceneNodeInstances::isReadOnlyMaterials() const {
	return ReadOnlyMaterials;
}


//! Creates a clone of this scene node and its children.
ISceneNode* CMeshSceneNodeInstances::clone(ISceneNode* newParent, ISceneManager* newManager) {
	if (!newParent)
		newParent = Parent;
	if (!newManager)
		newManager = SceneManager;

	CMeshSceneNodeInstances* nb = new CMeshSceneNodeInstances(Mesh, newParent,
